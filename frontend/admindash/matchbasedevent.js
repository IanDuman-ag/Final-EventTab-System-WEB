(function () {
  ...
  function updatePointsState() {
    ...
    $$('#points-editor input').forEach(input => { input.disabled = !enabled; });
  }

  function syncHiddenFields() {
    $('#team-ids').value = JSON.stringify(selectedTeams().map(team => team.id));
    $('#draw-order').value = JSON.stringify(drawOrder);
    const points = $$('#points-editor > div').map(row => {
      const inputs = $$('input', row);
      return { label: inputs[0].value.trim(), points: Number(inputs[1].value) };
    });
  ...
  function validateStep() {
    ...
    if (step === 4) {
      ...
      const pointInputs = $$('#points-editor input');
      if (pointInputs.some(input => input.value.trim() === '' || (input.type === 'number' && Number(input.value) < 0))) {
        error.textContent = 'Complete every championship points row with non-negative values.'; return false;
      }
    }
    return true;
  }
  ...
  function buildReview() {
    ...
    const points = JSON.parse($('#points-config').value || '[]');
    const pointsSummary = form.elements.apply_championship_points.checked
      ? points.map(point => `${point.label}: ${point.points} points`).join(' · ')
      : 'Automatic championship points disabled';
    const settings = [
      form.elements.auto_update_bracket.checked && 'Auto-update bracket',
      form.elements.allow_result_editing.checked && 'Editing before finalization',
      form.elements.require_faculty_confirmation.checked && 'Faculty confirmation'
    ...
    const rows = [
      ...
      ['Event Type', valueText('tournament_type')], ['Seeding Method', 'Random Draw'],
      ['Schedule', schedule], ['Faculty In Charge', valueText('faculty_account')],
      ['Championship Points', pointsSummary], ['Result Processing', settings],
  ...
  function fillEditor(event) {
    ...
    form.elements.auto_update_bracket.checked = event.auto_update_bracket;
    form.elements.allow_result_editing.checked = event.allow_result_editing;
    form.elements.require_faculty_confirmation.checked = event.require_faculty_confirmation;
    ...
    (event.points_config || []).forEach((point, index) => {
      const inputs = $$('#points-editor > div')[index] && $$('input', $$('#points-editor > div')[index]);
      if (inputs) { inputs[0].value = point.label; inputs[1].value = point.points; }
    });
